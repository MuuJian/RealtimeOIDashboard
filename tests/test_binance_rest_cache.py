...
class BinanceRestCacheTests(unittest.TestCase):
    ...
    def test_all_three_consumers_use_the_same_cached_responses(self):
        cache, client, _ = self.create_cache()
        ...
        signal_loader = SignalScanMarketSnapshotLoader(
            lambda *_args, **_kwargs: (_ for _ in ()).throw(
                AssertionError("direct shared request was not expected")
            ),
            threading.Event(),
            shared_rest_cache=cache,
        )
        ...
        signal_loader.load()
        cvd.refresh_universe(force=True)
