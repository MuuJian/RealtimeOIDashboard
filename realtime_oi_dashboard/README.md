...
浏览器端价格流与页面渲染分离：Binance WebSocket 先在独立数据源中合并同一帧的 ticker，再发布批次更新；筛选、排序、高 OI 信号和热力范围由 Web Worker 计算，Worker 不可用时自动回退主线程。UI 使用单一 `requestAnimationFrame` 队列提交更新，两个表格都会复用已有行节点，不通过 `innerHTML` 重建。
