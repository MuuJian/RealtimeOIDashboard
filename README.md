...
会输出每一页 CoinGecko 市值请求的成功或失败、该页匹配数量和当前累计加载数量；每轮结束后还会列出失败页与仍然缺少市值的 Binance 合约代码。完整 8 页均成功时，剩余缺失项通常表示币种不在 CoinGecko 市值前 2000，或 Binance 与 CoinGecko 的代码无法直接匹配。
